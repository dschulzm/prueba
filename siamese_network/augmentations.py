...
logger = logging.getLogger("train_utils_aug")
...
def build_augmentation_sequence_light():
    ...
    rotate_or_shear = iaa.OneOf([
        iaa.Affine(rotate=(-5, 5), fit_output=False),
        iaa.Affine(shear=(-4, 4))
    ])
    return iaa.SomeOf((3, None), [blur, multiply, temperature,
                                  rotate_or_shear], random_order=True)


def build_augmentation_sequence_medium(random_order=True):
    return iaa.SomeOf((0, None), [  # None to all augmentations
        iaa.OneOf([  # Affine
            iaa.Affine(rotate=(-5, 5), order=4, mode="constant"),
            iaa.Affine(shear=(-4, 4), order=4, mode="constant")
        ]),
        iaa.Multiply(mul=(.7, 1.4)),  # Brightness
        iaa.OneOf([  # Temperature
...
def build_augmentation_sequence_heavy(affine_chance=.5, crop_chance=0,
                                      brightness_chance=.5,
                                      temperature_chance=.35, blur_chance=.5,
                                      noise_chance=.35, dropout_chance=0,
                                      random_order=False):
    return iaa.SomeOf((0, None), [  # None to all augmentations
        iaa.Sometimes(affine_chance, [  # Affine
            iaa.SomeOf(1, [
                iaa.Affine(rotate=(-25, 25), order=4, mode="constant"),
                iaa.Affine(shear=(-16, 16), order=4, mode="constant"),
                iaa.Sequential([  # Apply both but less agressive
                    iaa.Affine(rotate=(-12, 12), order=4, mode="constant"),
                    iaa.Affine(shear=(-8, 8), order=4, mode="constant")
                ], random_order=True)
            ])
        ]),
        iaa.Sometimes(crop_chance, [  # Zoom effect
            iaa.Crop(percent=(.05, .1), keep_size=False)
        ]),
        iaa.Sometimes(brightness_chance, [  # Brightness
            iaa.SomeOf(1, [
                iaa.Multiply(mul=(.5, 1.5)),
                iaa.Add((-50, 50)),
                iaa.GammaContrast(gamma=(2, .5)),
                # iaa.imgcorruptlike.Brightness(severity=(1, 2))
            ])
        ]),
        iaa.Sometimes(temperature_chance, [  # Temperature
           iaa.SomeOf(1, [
                ...
                iaa.Sequential([  # Freezing
                    iaa.ChangeColorTemperature(40000),
                    iaa.ChangeColorTemperature(40000)
                ]),
            ])
        ]),
        iaa.Sometimes(blur_chance, [  # Blur
            iaa.SomeOf(1, [
                # iaa.GaussianBlur(sigma=(1, 3)),
                # iaa.AverageBlur(k=(3, 5)),
                iaa.MedianBlur(k=(3, 5)),
        ...
        iaa.Sometimes(noise_chance, [  # Noise
            iaa.SomeOf(1, [
        ...
        iaa.Sometimes(dropout_chance, [  # Drop areas
            iaa.CoarseDropout(p=(.1, .2)),
        ]),
    ], random_order=random_order)
